...
5. Check if there are staged changes: `git -C .worktrees/<feature-name> diff --cached --quiet`. If there are **no** staged changes, skip steps 6-8 (nothing to commit/push/PR) and proceed to "Stop the Ralph loop".
6. Commit: `git -C .worktrees/<feature-name> commit -m "feat(<feature-name>): <description>"`
7. Push: `git -C .worktrees/<feature-name> push -u origin $BRANCH_NAME`
8. Create PR (only if one doesn't already exist for `$BRANCH_NAME`):
   ```
...
9. Log the PR URL in the output
